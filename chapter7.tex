...
箱ひげ図は，観測値の中央値，第1四分位数，第3四分位数を示すことで，データの分布状況と中心的傾向を効果的に視覚化する．
さらに，箱ひげ図に外れ値を示すことで，データのばらつきを直接的に評価することができる．外れ値は以下のような手順で算出する．
\begin{enumerate}
  \item 各遅延時間でのボタンの押下時間間隔のデータを，被験者ごとにまとめる．
  \item 各遅延時間でのボタンの押下時間間隔のデータの中央値，第1四分位数，第3四分位数を算出する．
  \item 第3四分位数から第1四分位数を引いた値である四分位範囲(Interquartile Range, IQR)を計算する．
  \item (3)で求めた四分位範囲を1.5倍し，その数値分第3四分位数に足した値と，第1四分位数から引いた値の2つの値を上下の閾値とする．
  \item 閾値までに存在するデータの最も遠いところまで箱からひげを引く．
  \item 各データと閾値を比較し，4で求めた閾値の外にある値をすべて外れ値とし，ドットで個別に表す．
\end{enumerate}
